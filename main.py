...
app=Flask(__name__)
cors=CORS(app)
model22=pickle.load(open('LinearRegressionModel321.pkl','rb'))
car=pd.read_csv('Book111.csv')
car2=pd.read_csv('Book11.csv')
car3=pd.read_csv('new11.csv')
#FLASK LOGIN
from flask import Flask, render_template, request, redirect, url_for
...
# model11 =pickle.load(open('LinearRegressionModel1.pkl','rb')) #used car
# @app.route('/predict1',methods=['POST1'])
# @cross_origin()
# def predict1():
...
#     prediction1=model11.predict(pd.DataFrame(columns=['name', 'company', 'year', 'kms_driven', 'fuel_type'],
...
model=pickle.load(open('LinearRegressionModel321.pkl','rb')) # imported cars
...
model222=pickle.load(open('LinearRegressionModel333.pkl','rb')) #New Car
...
def predict2():
    ...
    prediction2=model222.predict(pd.DataFrame(columns=['name', 'company', 'year', 'suspension', 'fuel_type'],
